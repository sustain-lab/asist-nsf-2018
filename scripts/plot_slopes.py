...
for exp_name in exp_names:
    ...
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, xlim=(0, 60))
    plt.plot(fan, h3, color='b', marker='.', ms=12, label='Fetch 6 m, left')
    plt.plot(fan, h4, color='g', marker='.', ms=12, label='Fetch 6 m, right')
    plt.plot(fan, h6, color='r', marker='.', ms=12, label='Fetch 8.7 m')
    plt.legend(loc='upper left', fancybox=True, shadow=True)
    plt.grid()
    plt.xlabel('Fan speed [Hz]')
    plt.ylabel('Mean elevation [m]')
    plt.title(exp_name)
    plt.savefig('elevation_' + exp_name + '.png', dpi=100)
    plt.close(fig)
